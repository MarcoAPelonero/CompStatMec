...
public:
    // Constructor to initialize ProgressBar with total steps, bar width, and the character for progress
    ProgressBar(int total_steps, int bar_width = 50, std::string bar_char = "#");

    // Function to update the progress bar with the current step
    void update(int step);

    // Function to complete the progress bar at the end of the process
    void finish();
};
